...
description: 使用兼容 OpenAI Responses API 的识图模型进行云端识图，把图片内容转换为文字描述，使接入无视觉能力文本模型（如 DeepSeek）的 Codex 也能看图。支持一次请求传入多张图片批量识图。仅当当前模型无法直接查看图片时使用本技能（用户要求识图、看图、描述/分析/识别图片、图片内容问答、提取图片中的文字，或消息中出现 "Saved attachments:" 图片路径时）。若当前模型本身具备原生视觉能力，请直接查看图片并回答，不要调用本技能。
---

# 图像理解（image-understand）

本技能仅用于不具备原生视觉能力的文本模型（如 DeepSeek）。调用前先判断：当前模型是否能直接查看图片？若能，直接看图并回答，不要调用本技能；若不能，再运行 `scripts/vision.js`，把返回的文字结果整理给用户。
